  ...
  render() {
    return (
      <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    this.setState({
                      ...
                      validationError:
                        e.target.value === '' ? 'You must select a symbol' : '',
                    });
                  }}
                >
                  {this.state.syms.map((symbol, index) => (
                    <option key={index} value={symbol.value}>
                      {symbol.display}:{symbol.description}
                    </option>
                  ))}
                </select>
                <div
        ...
        <DisplayStocks
          name={this.state.selectedSymbol}
          from={Number(this.state.sDate.valueOf().toString().substring(0, 10))}
          to={Number(this.state.eDate.valueOf().toString().substring(0, 10))}
        />
      </div>
    );
  }
