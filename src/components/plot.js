...
class Plot extends Component {
    ...
    if (this.props.dataId !== prevProps.dataId) {
      axios({
        ...
        headers: {
          ...
          'x-rapidapi-host': 'finnhub-realtime-stock-price.p.rapidapi.com',
          'x-rapidapi-key':
            'a883fc58e6msh5ddecf03c777f85p16c295jsn47cb261e4668',
        },
        params: {
          to: to.toString(),
          symbol: dataId,
  ...
  render() {
    ...
    const { series1 } = {
      series1: [{ title: 'close price', data: data_c, color: 'red' }],
    };
    const { series2 } = {
      series2: [{ title: 'high price', data: data_h, color: 'orange' }],
    };
    const { series3 } = {
      series3: [{ title: 'low price', data: data_l, color: 'blue' }],
    };
    const { series4 } = {
      series4: [{ title: 'open price', data: data_o, color: 'green' }],
    };
    return (
      <div>
